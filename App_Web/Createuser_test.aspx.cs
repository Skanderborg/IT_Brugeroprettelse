    ...
    public partial class Createuser_test : System.Web.UI.Page
    {
        ...
        protected void Button_submit_Click(object sender, EventArgs e)
        {
            if (IsPageValid())
            {
                ...
                bool result = false;
                //service.CreateUser_test(CBOpus_medarbejdernr.SelectedValue,
                //                                TxbFornavn.Text,
                //                                TxbEfternavn.Text,
                //                                TxbVistnavn.Text,
                ...
                //                                RBIsRakat.SelectedIndex == 0,
                //                                RbRakatRole.SelectedValue,
                //                                Txb_rakat_profitomkoststed.Text,
                //                                Txb_rakat_konteringsansvarlig.Text,
                //                                RbIsKMDElev.SelectedIndex == 0,
        ...
        protected bool Oplysninger_om_medarbejder_is_valid()
        {
            ...
            if (int.TryParse(CBOpus_medarbejdernr.SelectedValue, out opus_id))
            {
                if (service.IsEmployeeInSofdCore(opus_id, null))
                {
        ...
        protected bool Ovrige_programmer_is_valid()
        {
            ...
            return nemid_is_valid && cura_is_valid && KMD && opus_okonomi_billagsbehandling && opus_lon_personale
                && targit_is_valid && kmdelev_is_valid && educaPersonale_isValid && rakat_isValid;
        }
        ...
        protected bool Cura_is_valid()
        {
            ...
            else
            {
                ...
                if (RbIsCura.SelectedIndex == 0)
                {
                    ...
                    {
                        errCuraRole.ForeColor = colNoErrLabel;
                        DdlCuraBrugerRolle.BorderColor = colNoErr;
                    }
                    else
                    {
                        errCuraRole.ForeColor = colErr;
        ...
        protected bool Targit_is_valid()
        {
            ...
            {
                errTargit.ForeColor = colErr;
                res = false;
            }
            else
            {
                errTargit.ForeColor = colNoErrLabel;
            }
            return res;
        }
        ...
        protected bool KMDelev_is_valid()
        {
            ...
            {
                errKMDelev.ForeColor = colErr;
                res = false;
            }
            else
            {
                errKMDelev.ForeColor = colNoErrLabel;
            }
            return res;
        }
        ...
        protected bool EducaPersonale_isValid()
        {
            ...
            {
                errEducaPersonale.ForeColor = colErr;
                res = false;
            }
            else
            {
                errEducaPersonale.ForeColor = colNoErrLabel;
                ...
                {
                    errEducaPersonale_Role.ForeColor = colErr;
                    res = false;
                }
                else
                {
                    errEducaPersonale_Role.ForeColor = colNoErrLabel;
                }
            }
            return res;
        }
        ...
        {
            bool res = true;
            if (RBIsRakat.SelectedIndex == -1)
            {
                errRBIsRakat.ForeColor = colErr;
                res = false;
            }
            else
            {
                errRBIsRakat.ForeColor = colNoErrLabel;
                if(RbRakatRole.SelectedIndex == -1)
                {
                    errRbRakatRole.ForeColor = colErr;
                    res = false;
                }
                else
                {
                    errRbRakatRole.ForeColor = colNoErrLabel;
                    if (RbRakatRole.SelectedValue == "B" || RbRakatRole.SelectedValue == "D")
                    {
                        if (Txb_rakat_profitomkoststed.Text.Length > 0)
                        {
                            Txb_rakat_profitomkoststed.BorderColor = colNoErr;
                        }
                        else
                        {
                            res = false;
                            Txb_rakat_profitomkoststed.BorderColor = colErr;
                        }
                    }
                    if (RbRakatRole.SelectedValue == "C")
                    {
        ...
        protected void RbRakatRole_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else if(RbRakatRole.SelectedValue == "C")
            {
                panel_rakat_profitomkoststed.Visible = false;
